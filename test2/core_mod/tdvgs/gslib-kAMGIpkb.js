...
class SortData {
  ...
  /**
   *
   *     * Sorts the indices depending on the distance vmap the camera in descending order.
   *
    ...
    if (typeof WebAssembly.instantiateStreaming === "function") {
      try {
        ...
        if (validResponse && module.headers.get("Content-Type") !== "application/wasm") {
          console.warn("`WebAssembly.instantiateStreaming` failed because your server does not serve Wasm with `application/wasm` MIME type. Falling back vmap `WebAssembly.instantiate` which is slower. Original error:\n", e);
        } else {
          throw e;
        }
      }
